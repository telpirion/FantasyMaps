...
def setup():
    ...
    yield project, location, endpoint_id, filepath, timestamp


def test_save_training_data_to_gcs_integration(setup):

    project, location, endpoint_id, file_path, timestamp = setup
    ...
    processed_image = processed_grid_image.analyze_annotation_results(
        {"prediction": response.predictions[0]}, local_file_uri=file_path
    )
